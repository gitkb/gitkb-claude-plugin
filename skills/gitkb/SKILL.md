...
5. **Always check numbering**: Run `git kb list <type> --all --json` before creating new documents
...
| `kb_commit` | Commit workspace changes | `message`, `pathspecs` |
...
| Tool | Description | Key Parameters |
|------|-------------|----------------|
| `kb_board` | Kanban board (JSON) | `type`, `tags`, `all`, `group_by`, `columns`, `sort_by`, `sort_direction` |
| `kb_context` | Bootstrap context bundle | `task`, `include_content`, `commit_limit`, `include_code_refs` |
| `kb_smart_context` | Task-aware code context with call graph | `task`, `token_budget`, `call_depth`, `include_callers`, `include_callees`, `min_score` |
...
git kb board                    # Tasks only (default), grouped by status
git kb board --all              # All document types
git kb board --group-by priority              # Group by priority instead of status
git kb board --group-by tags                  # Group by tags (docs in multiple columns)
...
git kb board --group-by priority --json       # JSON output for tooling
...
# JSON output for programmatic use (agents, scripts, CI)
git kb list --type task --json            # Structured list — no column truncation
git kb list --all --json                  # All documents as JSON
git kb board --json                       # Board as JSON object
git kb board --group-by priority --json   # Grouped board as JSON
```

**Agent tip:** When parsing CLI output programmatically, always pass `--json` to avoid width-dependent column truncation. You can also set `GITKB_COLUMNS=300` to widen tabular output, but `--json` is preferred.
...
4. Make changes and commit (always scope to your documents):
   ```
   kb_commit with message: "Progress on feature", pathspecs: ["tasks/my-task"]
   ```
